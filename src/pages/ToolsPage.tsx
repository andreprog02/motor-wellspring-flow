...
type SortField = 'part_number' | 'name' | 'aplicacao' | 'gerador' | 'quantity' | 'location_name';
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={filterLocation} onValueChange={setFilterLocation}>
              <SelectTrigger className="w-[180px]"><SelectValue placeholder="Todos os locais" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Todos os locais</SelectItem>
                {store.locations.map(loc => <SelectItem key={loc.id} value={loc.id}>{loc.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
