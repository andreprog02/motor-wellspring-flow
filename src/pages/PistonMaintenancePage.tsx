...
import { Plus, Trash2, Clock, Cylinder, Wrench } from 'lucide-react';
...
export default function PistonMaintenancePage() {
  ...
  // Fetch cylinder_components for selected equipment
  const cylinderComponents = useQuery({
    queryKey: ['cylinder_components', equipmentId],
    queryFn: async () => {
  ...
  const selectAllCylinders = () => {
    setSelectedCylinders(prev =>
      prev.length === cylinderNumbers.length ? [] : [...cylinderNumbers]
    );
  };
  ...
  const handleAddPeriodicity = () => {
    ...
    setPeriodicityEntries(prev => [...prev, {
      ...
      task: serviceType === 'inspection' ? 'Inspeção do pistão' : 'Substituição do pistão',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Update cylinder_components horimeter_at_install for replaced pistons
      ...
      toast({ title: 'Manutenção de pistão registrada com sucesso!' });
      setEquipmentId('');
      setHorimeter('');
      setNotes('');
      setSelectedItems([]);
  ...
  const history = logsData
    .filter(l => l.maintenance_type === 'piston_inspection' || l.maintenance_type === 'piston_replacement')
    .map(l => ({
      ...l,
  ...
  const maintenanceTypeLabel: Record<string, string> = {
    piston_inspection: 'Inspeção',
    piston_replacement: 'Substituição',
  };
  ...
  return (
    <AppLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Manutenção — Pistões</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Registre inspeções e substituições de pistões por cilindro
          </p>
        </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {cylinderNumbers.map(num => {
                    ...
                    return (
                      <label
                        key={num}
                        className={`flex flex-col items-center gap-1 rounded-lg border-2 p-3 cursor-pointer transition-colors min-w-[80px] ${
                          selectedCylinders.includes(num)
                            ? 'border-primary bg-primary/5'
                            : 'border-border hover:border-muted-foreground/50'
                        }`}
                      >
                        <Checkbox
                          checked={selectedCylinders.includes(num)}
                          onCheckedChange={() => toggleCylinder(num)}
                        />
                        <span className="text-sm font-medium">Cil. {num}</span>
                        {comp && (
                          <span className="text-xs text-muted-foreground">{hoursUsed}h uso</span>
                        )}
                      </label>
                    );
                  })}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Histórico de Manutenção de Pistões</CardTitle>
          </CardHeader>
                ...
                {history.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center text-muted-foreground py-8">
                      Nenhuma manutenção de pistão registrada
                    </TableCell>
                  </TableRow>
                ) : (
                  history.map(log => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>
                        <Badge variant={log.maintenance_type === 'piston_replacement' ? 'default' : 'secondary'}>
                          {maintenanceTypeLabel[log.maintenance_type] || log.maintenance_type}
                        </Badge>
                      </TableCell>
