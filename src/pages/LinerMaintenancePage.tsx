...
import { Plus, Trash2, Clock, Wrench, CircleDot } from 'lucide-react';
...
export default function LinerMaintenancePage() {
  ...
  const cylinderComponents = useQuery({
    queryKey: ['cylinder_components', equipmentId, 'liner'],
    queryFn: async () => {
  ...
  const handleAddPeriodicity = () => {
    ...
    setPeriodicityEntries(prev => [...prev, {
      ...
      task: serviceType === 'inspection' ? 'Inspeção da camisa' : 'Substituição da camisa',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: 'Manutenção de camisa registrada com sucesso!' });
  ...
  const history = logsData
    .filter(l => l.maintenance_type === 'liner_inspection' || l.maintenance_type === 'liner_replacement')
  ...
  const maintenanceTypeLabel: Record<string, string> = { liner_inspection: 'Inspeção', liner_replacement: 'Substituição' };

  return (
    <AppLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Manutenção — Camisas</h1>
          <p className="text-sm text-muted-foreground mt-1">Registre inspeções e substituições de camisas por cilindro</p>
        </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 items-end">
                ...
                <div className="space-y-1">
                  ...
                  <Input type="number" value={newPeriodInterval} onChange={e => setNewPeriodInterval(e.target.value)} placeholder="Ex: 8000" className="w-28 h-10" />
                </div>
        ...
        <Card>
          <CardHeader><CardTitle className="text-lg">Histórico de Manutenção de Camisas</CardTitle></CardHeader>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {history.length === 0 ? (
                  <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-8">Nenhuma manutenção de camisa registrada</TableCell></TableRow>
                ) : (
                  history.map(log => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell><Badge variant={log.maintenance_type === 'liner_replacement' ? 'default' : 'secondary'}>{maintenanceTypeLabel[log.maintenance_type] || log.maintenance_type}</Badge></TableCell>
