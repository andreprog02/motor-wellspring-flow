...
import { Plus, Trash2, Clock, Wrench, Zap } from 'lucide-react';
...
export default function SparkPlugMaintenancePage() {
  ...
  const cylinderComponents = useQuery({
    queryKey: ['cylinder_components', equipmentId, 'spark_plug'],
    queryFn: async () => {
  ...
  const handleAddPeriodicity = () => {
    ...
    setPeriodicityEntries(prev => [...prev, {
      ...
      task: serviceType === 'inspection' ? 'Inspeção da vela' : 'Substituição da vela',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: 'Manutenção de vela registrada com sucesso!' });
  ...
  const history = logsData
    .filter(l => l.maintenance_type === 'spark_plug_inspection' || l.maintenance_type === 'spark_plug_replacement')
  ...
  const maintenanceTypeLabel: Record<string, string> = { spark_plug_inspection: 'Inspeção', spark_plug_replacement: 'Substituição' };

  return (
    <AppLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Manutenção — Velas</h1>
          <p className="text-sm text-muted-foreground mt-1">Registre inspeções e substituições de velas por cilindro</p>
        </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 items-end">
                ...
                <div className="space-y-1">
                  ...
                  <Input type="number" value={newPeriodInterval} onChange={e => setNewPeriodInterval(e.target.value)} placeholder="Ex: 2000" className="w-28 h-10" />
                </div>
        ...
        <Card>
          <CardHeader><CardTitle className="text-lg">Histórico de Manutenção de Velas</CardTitle></CardHeader>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {history.length === 0 ? (
                  <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-8">Nenhuma manutenção de vela registrada</TableCell></TableRow>
                ) : (
                  history.map(log => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell><Badge variant={log.maintenance_type === 'spark_plug_replacement' ? 'default' : 'secondary'}>{maintenanceTypeLabel[log.maintenance_type] || log.maintenance_type}</Badge></TableCell>
